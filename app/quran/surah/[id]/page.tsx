...
import { fetchSurahDetail, type SurahDetailResponse } from "@/lib/api"
...
export default function SurahPage({ params }: SurahPageProps) {
  const { id } = use(params)
  const [surahData, setSurahData] = useState<SurahDetailResponse["data"] | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const response = await fetchSurahDetail(Number.parseInt(id))

        if (response.success) {
          setSurahData(response.data || null)
        } else {
          setError(response.error || "Failed to fetch surah")
        }
      } catch (err) {
  ...
  const createAndPlayAudio = async (surah: any, audioId: string, verseNumber: number | null = null) => {
    return new Promise((resolve) => {
      try {
        ...
        // Use verse-specific or surah audio URL
        const audioUrl = verseNumber
        ...
        const onLoadedData = () => {
          console.log(`✅ Audio loaded: ${surah.englishName}`)
          cleanup()

          audio
            .play()
            .then(() => {
              console.log(`▶️ Playing: ${surah.englishName}`)
              setIsPlaying(true)
              setAudioLoading(null)
        ...
        const onError = (e: Event) => {
          console.error(`❌ Load error: ${e.type}`)
          cleanup()
          resolve(false)
        }
        ...
        const onPause = () => {
          setIsPlaying(false)
          console.log(`⏸️ Paused: ${surah.englishName}`)
        }

        const onEnded = () => {
          console.log(`🏁 Finished: ${surah.englishName}`)
          stopAllAudio()
        }
        ...
        setTimeout(() => {
          if (audioLoading === audioId) {
            console.log(`⏰ Timeout for ${surah.englishName}`)
            cleanup()
            resolve(false)
          }
        }, 10000)
...
${window.location.origin}/quran/surah/${surahData.number}`
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            ...
            text: content,
            url: `${window.location.origin}/quran/surah/${surahData.number}`,
          })
