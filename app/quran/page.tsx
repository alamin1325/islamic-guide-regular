  ...
  const createAndPlayAudio = async (surah, audioId) => {
    return new Promise((resolve) => {
      try {
        ...
        // Use the most reliable audio source
        const audioUrl = `https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/${surah.id}.mp3`

        console.log(`🔊 Loading: ${audioUrl}`)

        // Set up event listeners
        const onLoadedData = () => {
          console.log(`✅ Audio loaded: ${surah.name}`)
          cleanup()
          ...
          audio
            .play()
            .then(() => {
              console.log(`▶️ Playing: ${surah.name}`)
              setIsPlaying(true)
              setAudioLoading(null)
        ...
        const onPause = () => {
          setIsPlaying(false)
          console.log(`⏸️ Paused: ${surah.name}`)
        }

        const onEnded = () => {
          console.log(`🏁 Finished: ${surah.name}`)
          stopAllAudio()
        }
        ...
        setTimeout(() => {
          if (audioLoading === audioId) {
            console.log(`⏰ Timeout for ${surah.name}`)
            cleanup()
            resolve(false)
          }
